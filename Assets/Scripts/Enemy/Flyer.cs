...
public class Flyer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        bool inRange = Mathf.Abs(distanceFromPlayer.x) <= attentionRange
                    && Mathf.Abs(distanceFromPlayer.y) <= attentionRange;
        enemyBase.isChasing = inRange || lookAtTarget != null;

        if (enemyBase.isChasing)
        {
            sawPlayer = true;
            float flySpeed = enemyBase.MovementSpeed;
            speed.x = Mathf.Sign(distanceFromPlayer.x) * flySpeed;
            speed.y = Mathf.Sign(distanceFromPlayer.y) * flySpeed;

            if (!NewPlayer.Instance.frozen)
            {
                if (shootsBomb)
                {
                    if (bombCounter > bombCounterMax) { ShootBomb(); bombCounter = 0; }
        ...
        if (avoidGround)
        {
            ...
            if (rayCastHit.collider != null) speed.x = -(Mathf.Abs(speed.x));
            ...
            if (rayCastHit.collider != null) speed.y = Mathf.Abs(speed.x);
        }

        if (lookAtTarget != null) LookAt2D();

        if (lifeSpan != 0)
        {
            if (lifeSpanCounter < lifeSpan) lifeSpanCounter += Time.deltaTime;
            else enemyBase.Die();
        }
    }
    ...
    {
        Instantiate(bomb, new Vector3(transform.position.x, transform.position.y, transform.position.z), Quaternion.identity, null);
    }
}